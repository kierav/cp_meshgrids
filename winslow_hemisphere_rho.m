...
[Dxb,Dxf,Dyb,Dyf,Dzb,Dzf] = firstderiv_upw1_3d_matrices(x1d,y1d,z1d,band,band);
[Axb,Axf,Ayb,Ayf,Azb,Azf] = average_upw1_3d_matrices(x1d,y1d,z1d,band,band);
E = interp3_matrix(x1d,y1d,z1d,cpxgb,cpygb,cpzgb, p, band);
Eu = E;
Ev = Eu;
I = speye(size(Eu));
...
beta = 2;   alpha = 1;
xc_bar = 0.5;   yc_bar = -0.6;  zc_bar = 0.6;
[xc,yc,zc] = cpf(xc_bar,yc_bar,zc_bar,R);
rho_bar = 1-(beta-1)*exp(-alpha*((xg-xc).^2+(yg-yc).^2+(zg-zc).^2));
rho = E*rho_bar;
% rho = rho_bar;
% rho = ones(size(xg));
...
for kt = 1:numtimesteps
  ...
  rhsu = Dxb*((Axf*rho).*(Dxf*u))+Dyb*((Ayf*rho).*(Dyf*u))+Dzb*((Azf*rho).*(Dzf*u));
  rhsv = Dxb*((Axf*rho).*(Dxf*v))+Dyb*((Ayf*rho).*(Dyf*v))+Dzb*((Azf*rho).*(Dzf*v));
  unew = u + dt*( Eu*rhsu - lambda*(u - Eu*u) );
  vnew = v + dt*( Ev*rhsv - lambda*(v - Ev*v) );
  u = unew;
  v = vnew;
